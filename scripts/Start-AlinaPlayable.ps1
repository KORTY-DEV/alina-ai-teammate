...
if (-not $FactorioExe) { throw 'Factorio executable не найден.' }
...
# Первый запуск делает отдельную постоянную игровую копию. Дальнейшие запуски продолжают именно её.
...
if (-not (Test-Path -LiteralPath $playableSave)) {
    ...
    if (-not $SourceSave -or -not (Test-Path -LiteralPath $SourceSave)) {
        throw 'Не найден исходный сейв для создания Alina-Playable.zip.'
    }
...
try {
    ...
    $clientArgs = '--mod-directory "{0}" --enable-lua-udp {1} --load-game "{2}" --no-log-rotation' -f `
        $modsRoot, $FactorioUdpPort, $playableSave
    Write-Host 'Запускаю Factorio в обычном SINGLEPLAYER. Локальный multiplayer/RCON больше не используется.' -ForegroundColor Cyan
    Write-Host 'Когда загрузится сейв, Bridge в окне Алины должен стать connected.'
